...
function check_whoami() {
  ...
  if [ ! $brk ]; then
    ...
    git branch liuhy 2>/dev/null
    git checkout liuhy
    check "#git checkout liuhy"
  fi
}
...
function check_renamed() {
  res=$(git status --porcelain | grep "^R " | wc -l)
  if [ $res -eq 0 ]; then
    return 1
  fi
  return 0
...
function check_newfile() {
  res=$(git status --porcelain | grep "^A " | wc -l)
  if [ $res -eq 0 ]; then
    return 1
  fi
  return 0
...
function check_delfile() {
  res=$(git status --porcelain | grep "^D " | wc -l)
  if [ $res -eq 0 ]; then
    return 1
  fi
  return 0
